...
class BoletaElectronica:
    ...
    def leer_archivo_entrada(self, archivo: str) -> pd.DataFrame:
        ...
        try:
            if archivo.endswith(('.xlsx', '.xls')):
                ...
                raise ValueError(f"Formato de archivo no soportado: {archivo}. Solo se aceptan .xlsx, .xls")

            # Eliminar espacios en los nombres de las columnas
            df.columns = df.columns.str.strip()

            # Validar columnas requeridas
            columnas_requeridas = ['SERIE', 'NUMERO', 'FECHA', 'CLIENTE', 'MONEDA', 'TOTAL', 'PRODUCTO', 'PRECIO', 'CANTIDAD', 'IMPORTE']
            ...
            df_interno = pd.DataFrame({
                'numero_documento': df['SERIE'].astype(str) + '-' + df['NUMERO'].astype(str),
    ...
    def dividir_item_grande(self, item: Dict[str, Any], limite: float) -> List[Dict[str, Any]]:
        """
        ...
        """
        subtotal_item = self.calcular_subtotal_item(item['cantidad'], item['precio_unitario'])
        ...
        for i in range(num_partes):
            ...
            item_dividido['subtotal'] = self.calcular_subtotal_item(cantidad_por_parte, item['precio_unitario'])
            items_divididos.append(item_dividido)
        
        return items_divididos
    
    def optimizar_empaquetado(self, items: List[Dict[str, Any]], limite: float) -> List[List[Dict[str, Any]]]:
        """
        ...
        for item in items:
            ...
            while cantidad_restante > 0:
                ...
                if boletas and sum(i['subtotal'] for i in boletas[-1]) + subtotal <= limite:
    ...
    def generar_boleta(self, items: List[Dict[str, Any]], documento_origen: str) -> Dict[str, Any]:
        """
        ...
        """
        # Calcular totales
        subtotal = sum(item.get('subtotal', self.calcular_subtotal_item(item['cantidad'], item['precio_unitario'])) 
                      for item in items)
    ...
    def procesar_documento(self, df_documento: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for _, row in df_documento.iterrows():
            item = {
                ...
                'subtotal': self.calcular_subtotal_item(float(row['cantidad']), float(row['precio_unitario']))
            }
            items.append(item)
        ...
        self.logger.info(f"Documento {numero_doc}: {len(boletas_generadas)} boletas generadas")
        
        return boletas_generadas
    
    def procesar_archivo_completo(self, archivo_entrada: str) -> None:
        """
    ...
    def generar_archivos_salida(self) -> None:
        ...
        try:
            ...
            for boleta in self.boletas_generadas:
                ...
                boletas_doc = [b for b in self.boletas_generadas if b['documento_origen'] == doc]
                # Preparar datos para Excel
                datos_excel = []
                for b in boletas_doc:
                    serie_numero = b['numero']
                    fecha = b['fecha_emision']
                    cliente = b['cliente']['nombre'] if isinstance(b['cliente'], dict) else b['cliente']
                    moneda = 'Soles'
                    total = b['total']
                ...
                columnas_salida = ['SERIE-NUMERO', 'FECHA', 'CLIENTE', 'MONEDA', 'SUBTOTAL', 'IGV', 'TOTAL', 'ANEXO', 'PRODUCTO', 'PRECIO', 'CANTIDAD', 'IMPORTE']
